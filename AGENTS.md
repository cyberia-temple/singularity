...
- **The vault has two kinds and never blurs them** (`resources/js/lib/wallet/vault.ts`, `tests/Frontend/WalletVaultTest.mjs`). Sealed: AES-256-GCM over a PBKDF2-SHA-256 key, the record this file was written for. Unprotected: `protection: 'none'` and the same JSON document in the clear, for somebody who declined a password at setup — which is a real choice and is offered as one. Rules that must not be relaxed: the open record carries **no `kdf`, no `salt`, no `iv` and no `ciphertext`** (encrypting under a key stored next to it would protect nobody while letting the UI keep claiming encryption, so the honest form is plaintext plus a record that admits it); `saveOpenVault()` stays a separate function from `saveVault()` so that storing a seed in the clear can never be reached by a password argument that arrived `undefined`; `openUnprotectedVault()` is **synchronous**, because the page picks the lock screen from whether the vault is open and an `await` there flashes a password prompt at somebody who has none; and `lock()` plus the idle timer both refuse an unprotected vault, since a lock screen whose answer does not exist is not a lock. `protectVault()` upgrades in place with every account intact, and `VaultContents.backedUp` (absent = true, because every older vault passed a mandatory check) is what Security warns about until the phrase has actually been copied out.
...
**Both apps are the Cyberia wallet first.** They launch on `CYBERIA_APP_PATH` (default `/wallet`), and that route drops the site header and footer and fills the frame — in every container, a browser tab included (`resources/js/layouts/WalletFrameLayout.vue`, which `app.ts` now picks for `Wallet` unconditionally); everything else in the shell keeps the normal site chrome. The **masthead** is drawn only inside a shell, where it is the title bar of a frameless window and carries the link back to the site; in a browser it was a second heading and a second language button over a page that had a site header too, and the link out is on the wallet's own Settings screen instead. `/wallet` is a public route so the app works straight after install — the keys are browser-side and were never the server's to gate; signing in only adds the XMR payout binding. `CYBERIA_APP_PATH` is validated as a same-origin absolute path: a full URL or `//host` falls back to `/wallet` instead of repointing the app at another origin.
...
On the site, `resources/js/lib/telegram.ts` decides the frame from the launch parameters Telegram appends to the URL, synchronously, so `initializeNativeShell()` returns `'telegram'` before anything paints — the layout is the wallet's own either way now, but the masthead and the shell's chrome still turn on this. The SDK is fetched from telegram.org **only** inside the frame. Rules that are load-bearing rather than cosmetic:
